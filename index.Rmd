...
annual_change <- function(data, periods) {
  data |>
    dplyr::arrange(date) |>
    dplyr::mutate(value = 100 * (value / dplyr::lag(value, n = periods) - 1))
}
...
```{r helpers}
# dcatalogo <- bcchr::metadata()
# dcatalogo |> dplyr::filter(stringr::str_detect(spanish_title, "IMACEC")) |> View()
get_chart <- function(timeseries = "F049.DES.TAS.INE9.10.M", # "F032.PIB.FLU.R.CLP.EP18.Z.Z.0.T",
  ...
  # post_fun   <- \(x) annual_change(x, periods = 4)
  # format_num <- \(x) percent(x/100, accuracy = 0.01)
  # ... <- NULL
  
  data <- bcchr::get_series(timeseries, from = firstdate, to = lastdate)
  ...
  if (is.null(subtitle)) {
    subtitle <- bcchr::describe_series(timeseries, verbose = FALSE)$spanish_title[[1]]
  }
...
get_chart(
  ...
  name = "IMACEC",
  post_fun = \(x) annual_change(x, periods = 12),
  format_num = \(x) percent(round(x, 2)/100)
)
```
